...
assert.strictEqual(
  __test.requiredFieldItemsOwnerForScope({ rules: { requiredFields: { fallbackOwner: "王谦" } } }, "李晶晶", true),
  "王谦",
  "已授权的兜底查看者应读取配置兜底人的任务集合"
);
assert.strictEqual(
  __test.requiredFieldItemsOwnerForScope({ rules: { requiredFields: { fallbackOwner: "王谦" } } }, "李晶晶", false),
  "李晶晶",
  "普通个人范围仍应读取当前用户任务"
);

assert.deepStrictEqual(
  __test.requiredFieldFallbackViewerNames({ rules: { requiredFields: { fallbackOwner: "王谦" } } }),
  ["王谦", "李晶晶"],
  "兜底查看者应保留配置的兜底人并增加李晶晶"
);
assert.strictEqual(
  __test.canReadRequiredFieldFallbackScope({ rules: { requiredFields: { fallbackOwner: "王谦" } } }, "李晶晶"),
  true,
  "李晶晶可以读取兜底任务"
);
assert.strictEqual(
  __test.canReadRequiredFieldFallbackScope({ rules: { requiredFields: { fallbackOwner: "王谦" } } }, "李东"),
  false,
  "非兜底查看者不能读取兜底任务"
);
