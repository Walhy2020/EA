...
async function main() {
  ...
  try {
    ...
    assert.deepEqual(mock.sheets().map((sheet) => sheet.title), ["Sheet1", "第二张表"]);
    assert.equal(mock.registry.length, 1);
    assert.equal(mock.registry[0].docType, 4);
    assert.equal((await module.importExcel(input)).duplicate, true);
    assert.equal((await moduleFor(mock, "success").importExcel(input)).duplicate, true, "dedupe survives restart");
    assert.equal(mock.creates(), 1);
    assert.equal((await module.importExcel({ ...input, sender: {} })).ok, false);
    const failure = mockApi({ failWrite: true });
    const failed = await moduleFor(failure, "failure").importExcel(input);
    assert.equal(failed.status, "partial_import");
    assert.match(failed.text, /未完成/);
    assert.match(failed.text, /https:\/\/doc.weixin.qq.com/);
    assert.ok(!failed.text.includes("private remote message"));
    const corrupt = await moduleFor(mockApi({ corruptRead: true }), "corrupt").importExcel(input);
    assert.equal(corrupt.status, "partial_import");
    assert.equal((await moduleFor(mockApi({ documentedShape: true }), "documented-shape").importExcel(input)).ok, true);
    const invalid = mockApi();
    const invalidResult = await moduleFor(invalid, "invalid").importExcel({ ...input, download: async () => ({ buffer: fixture({ missingResult: true }), filename: "bad.xlsx" }) });
    assert.equal(invalidResult.ok, false);
    assert.equal(invalid.creates(), 0, "validate before creating document");
    ...
    assert.match(controller.capture(event("转成普通表格")).text, /5分钟/);
    ...
    try {
      ...
      assert.equal(replies.at(-1).text, "import done");
    ...
    console.log("Ordinary spreadsheet import OK: content, xlsx/xls, formulas, limits, API, readback, dedupe, identity and bot flow");
  } finally {
