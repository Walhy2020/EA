...
const source = require("../docs/monitor-rules/V0007.source.json");
const rules = getDevProgressSettings().rules;
const config = rules.requiredFields;
assert.strictEqual(config.sourceVersion, "V0007");
...
expectEvery(inspect("UI日方时间", { UI进度: "日方制作中" }), (item) => item.reason === "empty_value");
expectEvery(inspect("UI日方时间", {
  UI进度: "已提交", UI日方时间: "2026-09-10", 美术截止日期: "2026-09-09"
}), (item) => item.reason === "date_after_deadline");
expectEvery(inspect("UI剩余时间", { UI进度: "已提交", UI剩余时间: 1 }, "内网测试中"),
  (item) => item.reason === "number_above_maximum");
expectEvery(inspect("UI剩余时间", { UI进度: "已提交", UI剩余时间: 0 }, "内网测试中"), (item) => !item.missing);
...
console.log("V0007 source parity and conditional required-field checks passed");
