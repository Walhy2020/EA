...
> 注意：下方是 2026-06-11 的历史快照，不再作为需求监控规则的字段依据。每次规则源版本变化，必须通过企业微信 `smartsheet/get_fields` 重新读取“需求池”实时列结构后再执行监控。2026-09-01 对照 `V0001` 时，实时需求池为 61 列，新增“特效制作耗时/剩余”“动作制作耗时/剩余”，已删除“动效制作耗时/剩余”，并且没有规则表仍引用的“监修时间”。运行时会跳过不存在的规则目标列；列结构读取失败时中止本轮扫描并保留上次有效缓存。
